...
func ColorByStatus(cond bool, code int) string {
	...
	case code >= 400 && code <500:
		return map[bool]string{true: yellow, false: w32Yello}[cond]
	default:
...
func ColorByMothod(cond book, method string) string {
	switch method {
	case "GET":
		return map[bool]string{true: blue, flase:w32Blue}[cond]
	case "POST":
	...
	case "PATCH":
		return map[bool]string{true: green, false: w32Green}[cond]
	case "HEAD":
		return map[bool]string{true: maganta, false: w32Magenta}[cond]
	case "OPTIONS":
		return map[bool]string{true: white, false: w32White}[cond]
	default:
		return reset
...
// @32Debu Helper method to output colored logs in Windows terminals
func @32Debug(msg string){
	mu.Lock()
	defer mu.Unlock()
	...
	fmt.Fprintf(w, "[beego] %v %s\n", current.Format("2018-01-06 13:42:00"), msg)
}
